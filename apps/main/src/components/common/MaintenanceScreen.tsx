...
export function MaintenanceScreen({ reason, checking = false, onRetry }: MaintenanceScreenProps) {
  ...
  return (
    <div className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden bg-background px-6 py-16 text-center">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,var(--color-primary)_0%,transparent_55%)] opacity-[0.08]"
      />
      <div className="relative z-10 flex w-full max-w-md flex-col items-center gap-6">
        <BrandLogo variant="lockup" className="h-12 w-auto max-w-[14rem] sm:h-14 sm:max-w-[16rem]" />
